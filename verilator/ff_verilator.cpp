...
#define THROW_TIME	(2000000*2) //25mhz = *2
         ...
         int throwbutton= joystick_throw();
         int testbutton= joystick_test();

         int joyx = joystick_x();
         int joyy = joystick_x();
         //printf("joyx %d joyy %d throw %d test %d\n",joyx,joyy,throwbutton,testbutton); 
	  if (throwbutton) {
           //printf("assert 2 - throw\n");
	   sw_assert(2);
	  }
	  else
	   sw_deassert(2);
